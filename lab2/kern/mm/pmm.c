...
void pmm_init(void) {
    // 初始化物理内存管理器（Physical Memory Manager）
    init_pmm_manager();

    // 检测物理内存空间，保留已使用的内存，并使用 pmm->init_memmap 创建空闲页面列表
    page_init();

    // 使用 pmm->check 验证 pmm 中的分配/释放函数的正确性
    check_alloc_page();

    // 引入 boot_page_table_sv39 数组，用于设置虚拟内存的页表
    extern char boot_page_table_sv39[];
    satp_virtual = (pte_t*)boot_page_table_sv39;

    // 获取 satp_virtual 在物理内存中的地址
